...
import cloudyImage from '../assets/images/cloudy-img.png';
...
export default function MainCard() {
  ...
  const getTempHum = async () => {
    const response = await fetch(`${BACKEND_APP}/dhtRain`, {
      method: 'GET',
  ...
  const getRoofCondition = async () => {
    const response = await fetch(`${BACKEND_APP}/roof`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await response.json();
                  ...
                  <div className='col-sm-12 d-flex justify-content-between'>
                    <div className='row'>
                      <div className='col-sm-3 d-flex align-items-center'>
                        <img
                          src={weather === 'Cloudy' ? cloudyImage : sunnyImage}
                          alt='weather'
                        />
                      </div>
